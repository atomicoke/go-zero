...
* 通过ddl生成
    ...
    ```shell script
    goctl modelv2 mysql ddl -src="./*.sql" -dir="./sql/model" -c
    ```
...
* 通过datasource生成
    ...
    ```shell script
    goctl modelv2 mysql datasource -url="user:password@tcp(127.0.0.1:3306)/database" -table="*"  -dir="./model"
    ```
...
* 带缓存模式
  * ddl
	...
	```shell script
	goctl modelv2 mysql -src={patterns} -dir={dir} -cache
	```
  ...
  * datasource
	...
	```shell script
	goctl modelv2 mysql datasource -url={datasource} -table={patterns}  -dir={dir} -cache=true
	```
...
* 不带缓存模式
  ...
  * ddl
      ...
      ```shell script
        goctl modelv2 -src={patterns} -dir={dir}
      ```

  * datasource
  
      ```shell script
        goctl modelv2 mysql datasource -url={datasource} -table={patterns}  -dir={dir}
      ```
