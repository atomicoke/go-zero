...
import (
	...
	"github.com/zeromicro/go-zero/tools/goctl/modelv2/sql/parser"
	"github.com/zeromicro/go-zero/tools/goctl/modelv2/sql/template"
	modelutil "github.com/zeromicro/go-zero/tools/goctl/modelv2/sql/util"
...
// 读取svc文件
func readSvcFile(dir string) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, "read file error:"+filePath)
	}
...
// 写入svc文件
func writeSvcFile(dir string, content string) error {
	fileName := "servicecontext.go"
	filePath := path.Join(dir, fileName)
	// 写入文件
	err := os.WriteFile(filePath, []byte(content), 0666)
	if err != nil {
		return errors.Wrap(err, "write file error:"+filePath)
	}
	return nil
}
