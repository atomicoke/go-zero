...
import (
	...
	"github.com/zeromicro/go-zero/tools/goctl/modelv2/mongo/template"
...
// Context defines the modelv2 generation data what they needs
type Context struct {
	Types  []string
	Cache  bool
	Easy   bool
...
// Do executes modelv2 template and output the result into the specified file path
...
func generateModel(ctx *Context) error {
	for _, t := range ctx.Types {
		...
		if err = util.With("modelv2").Parse(text).GoFmt(true).SaveTo(map[string]any{
...
func generateCustomModel(ctx *Context) error {
	for _, t := range ctx.Types {
		...
		output := filepath.Join(ctx.Output, fn+".go")
		err = util.With("modelv2").Parse(text).GoFmt(true).SaveTo(map[string]any{
			"Type":      stringx.From(t).Title(),
			"lowerType": stringx.From(t).Untitle(),
