...
import (
	...
	"github.com/zeromicro/go-zero/tools/goctl/modelv2/mongo"
	"github.com/zeromicro/go-zero/tools/goctl/modelv2/sql/command"
)

var (
	// Cmd describes a modelv2 command.
	Cmd             = cobrax.NewCommand("modelv2")
	mysqlCmd        = cobrax.NewCommand("mysqlv2")
	ddlCmd          = cobrax.NewCommand("ddlv2", cobrax.WithRunE(command.MysqlDDL))
	datasourceCmd   = cobrax.NewCommand("datasourcev2", cobrax.WithRunE(command.MySqlDataSource))
	pgCmd           = cobrax.NewCommand("pgv2", cobrax.WithRunE(command.PostgreSqlDataSource))
	pgDatasourceCmd = cobrax.NewCommand("datasourcev2", cobrax.WithRunE(command.PostgreSqlDataSource))
	mongoCmd        = cobrax.NewCommand("mongov2", cobrax.WithRunE(mongo.Action))
)
